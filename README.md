...
| Раздел | Что внутри |
|---|---|
| 📖 **Буквы** | Три подрежима: отправка ключом (тап/пробел, с живым сайдтоном), приём на слух (играет символ → нажми на экранной клавиатуре) и **ритм ключа** (сравнивает реальные длительности точки/тире/пауз с идеалом 1:3:1 — скорость свободно регулируется, идеал пересчитывается под неё; 5 точных повторов подряд оттачивают ритм буквы и дают разовый XP). Три набора для отправки/ритма — «Латиница + цифры», порядок Коха, кириллица (33 буквы, своя таблица кодов, свои слоговые напевы); отдельный набор «Кириллические символы» для приёма на слух |
