...
| 🏆 **Достижения** | 20 ачивок, условия хранятся в БД (`achievements`) — можно добавлять новые без правки кода |
...
│   └── js/                   # morse-data, abbreviations, words, audio, audio-settings, display-settings, input, signal, progress, страницы
...
- **`assets/js/words.js`** — банк текстов для режима «Реальные слова»:
  `COMMON_WORDS` (частые английские слова) и `RADIO_PHRASES` (куски
  радиообмена). Всё только латиницей и цифрами: символы обязаны быть
  известны `MORSE_CODE`, иначе при воспроизведении они молча пропустятся.
