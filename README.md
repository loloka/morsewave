...
| **Вторжение** (BETA) | **+1 XP** за каждого пришельца (+2 XP за Танк) и бонус `×2` за добивание Босса, плюс разовый бонус победы волны (10–130 XP) | Флэт-ставка за попадание — фарм не выгоднее обычного приёма на слух. Боссы (группы символов) при добивании дают дополнительный опыт, равный их длине ×2 (соразмерно затраченным усилиям). Бонус победы начисляется только за полностью пройденную волну (100 попаданий) и складывается из остатка HP базы (1:1) и средней скорости реакции по всей волне (10–30 XP). |
