...
| 👤 **Аккаунт** | Опционально: таблица лидеров (публикация только вручную), синхронизация прогресса между устройствами, восстановление пароля по почте |
...
├── CHANGELOG.md               # история изменений (MAJOR.MINOR.PATCH, свежие сверху)
...
└── settings.php                # 301-редирект на account.php (настройки переехали в профиль, 2.24.0)
```
...
| **Задание дня** | **+50 XP** бонусом | Один раз в день. Параметры сессии (число групп, длина, скорость) должны точно совпадать с заданием **и точность должна быть не ниже 50 %** — прокликать пустые ответы не выйдет (фикс 2.26.0). |
...
Полная история — в [CHANGELOG.md](CHANGELOG.md) (формат MAJOR.MINOR.PATCH,
...
- Письма подтверждения шлются через **Resend API** (`config/mail.php`,
  ...
  если понадобится SMTP-путь снова, см. записи 2.20.0/2.21.0 в [CHANGELOG.md](CHANGELOG.md).
